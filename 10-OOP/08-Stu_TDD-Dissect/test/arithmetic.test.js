...
describe("Arithmetic", () => {
  ...
  describe("multiply", () => {
    it("should return a new 'Arithmetic' object that has an updated 'number' value", () => {
      const obj = new Arithmetic(3).multiply(2);
      expect(obj instanceof Arithmetic).toEqual(true);
    });
    it("should return the product of two numbers as a new 'Arithmetic' object", () => {
      const num = 3;
      const multiplier = 2;
      const product = multiplier * num;
      const number = new Arithmetic(num).multiply(multiplier).number;
      expect(number).toEqual(product);
